---
title: 'The Homelab'
summary: 'A UniFi network running a NixOS fleet from one flake: workstation, services host, media, a libvirt security lab, a staging/CI box that moonlights as a Minecraft server, and an off-site Linode node fronting it, all deployed with a single colmena apply. One Ubuntu NAS stays off NixOS on purpose.'
status: 'active'
year: 'ongoing'
...
| `hacktop`     | Staging, the self-hosted CI runner, and the fleet's Minecraft server       |
...
The estate doesn't stop at the house. `cloud1` is a Linode node,
...
managing the house and the cloud. It has already lived one full lifecycle:
stood up, torn down when it had no job, and rebuilt from the same code the
day it got one. That round trip is the part worth noticing — when the
infrastructure is code, decommissioning costs nothing and coming back is a
`terraform apply` and a `colmena apply`. The SIEM went declarative too: the
fleet moved off **Wazuh** onto a leaner **Loki/Alloy** stack (`siem-lite`) —
Alloy ships every host's journal to Loki on `mgmt`, and Alertmanager pushes
to a self-hosted ntfy topic. That cutover was gated, because `mgmt` carries
the LAN's DNS and PKI and a bad deploy there is felt everywhere.

## A public game, no open home ports

The job that brought `cloud1` back: **`play.briggsbastian.com`**, an *All
the Mons* modpack server (Minecraft 1.21.1, NeoForge, 373 mods) I run for
my friends. It lives on `hacktop` — the box with the RAM headroom — as a
fully declarative **nix-minecraft** service, the modpack's server files
pinned into the Nix store like any other dependency. Publishing it never touches the home
firewall: `hacktop` dials **out** to `cloud1` over WireGuard and holds the
tunnel open with keepalives, and nftables on the Linode DNATs public
`:25565` traffic down the tunnel. No inbound rule at home, the home IP
never published, and every drive-by scan on the internet lands on the
cloud box instead of the house — all for ~10–25 ms of added latency. Why
I like this shape, and where it stops scaling, is a
[garden note](/garden/the-cloud-box-is-a-bouncer/).
