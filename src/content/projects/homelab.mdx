...
the Mons* modpack server (Minecraft 1.21.1, NeoForge, 373 mods) that runs
on `hacktop` — the box with the RAM headroom — as a fully declarative
**nix-minecraft** service, the modpack's server files pinned into the Nix
store like any other dependency. Publishing it never touches the home
...
cloud box instead of the house — all for ~10–25 ms of added latency.
